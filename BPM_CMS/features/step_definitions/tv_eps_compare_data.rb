def tv_eps_compare_values
	...
	series_id = $driver.find_element(:xpath,".//*[@id='series-id-value']/a")
	series_state = series_id.displayed?
	unless series_state == true
		puts "SeriesId field is not displayed"
	else 	
		 if series_id.text != "#{$bpm_element["#{$pid[0]}"][:showID]}"
			puts "Series_id entered in CMS is not same as mentioned in XML" 
		else
			puts "Series_id entered in CMS is same as mentioned in XML"
		end
	end

	series_title = $driver.find_element(:xpath,".//*[@id='series-title-value']")
	series_title_state = series_title.displayed?
	unless series_title_state == true
		puts "series_title field is not displayed"
	else 	
		 if series_title.text != "#{$bpm_element["#{$pid[0]}"][:Title]}"
			puts "Series_title entered in CMS is not same as mentioned in XML" 
		else
			puts "Series_title entered in CMS is same as mentioned in XML"
		end
	end
	...
	unless season_title_state == true
		...
		if season_title.text != expected_st
			...
			$bpm_element["#{$pid[0]}"][:season_title_state] = "#{season_title_state}"
		end
	end	

	season_number = $driver.find_element(:xpath,".//*[@id='season-number-value']")
	...
	if episode_number.attribute("value") != "#{$bpm_element["#{$pid[0]}"][:EpisodeNumber]}"
	...
	if episode_title.attribute("value") != "#{$bpm_element["#{$pid[0]}"][:EpisodeTitle]}"
	...
	imdb_no = imdbid.send_keys("0103359")
